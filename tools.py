...
def _explain_http_error(exc: Exception) -> str:
    ...
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        return f"the backend {exc.response.status_code}: {detail}"
...
class NotesExtension(Extension):
    ...
    Backend: the backend (the backend host:8097). Wire contract is frozen — this class
    ...
    async def list_notes(
        ...
        try:
            ...
            # AND-tag client fallback: older the backend ignored ?tags= and
