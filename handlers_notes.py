...
# the backend caps `limit` at 200 via FastAPI `Query(le=200)`. Keep the extension
...
async def fn_list_notes(ctx, params: ListNotesParams) -> ActionResult:
    ...
    try:
        ...
        # of backend version. Once the backend ships server-side tag filtering,
        ...
        # Prefer true DB-wide count if the backend provides it (new field after
